...
#ifdef __cplusplus  //利用__cplusplus宏实现C和C++头文件共用
extern "C" {
#endif
...
extern void ReadHMM(FILE *fp, HMM *phmm);
...
void ReReadSequence(FILE *fp, int *pT, int *O);
void PrintSequence(FILE *fp, int T, int *O);
void GenSequenceArray(HMM *phmm, int seed,int T, int *O, int *q);//zy
